"""
fetcher_finland.py — Finnish cultural heritage via Europeana (COUNTRY:finland)

Uses the Europeana API filtered by COUNTRY:finland to surface records
from Finnish museums, archives and national libraries.
Over 144,000 records available.
...
TOPICS = [
    ...
    "Finnish Reformation",
    "Nordic history",
    "Baltic history Finland",
    "Viking age Finland",
    "Finnish language",
    "Finnish art history",
    "Russo-Finnish relations",
    "Finland Russia history",
    "Finnish migration history",
]
    ...
    for topic in TOPICS:
        ...
        while query_inserted < MAX_PER_QUERY:
            ...
            try:
                ...
                print(f"  [FI] Request error for '{topic}': {e}")
                break

            if resp.status_code != 200:
                print(f"  [FI] HTTP {resp.status_code} for '{topic}'")
                break

            try:
                data = resp.json()
            except ValueError as e:
                print(f"  [FI] JSON parse error: {e}")
                break
            ...
            for item in items:
                try:
                    ...
                    tag_list = ["Finland", "Europeana Finland", "Finna", topic]
                    if provider:
                    ...
                    ok = insert_record(conn, source_id, {
                        ...
                        "source_url":  src_url,
                        "external_id": f"eu-fi-{item_id.replace('/', '-')}",
                        "date_text":   date_text,
                        "record_type": rtype,
                        "tags":        tags,
                        "region":      "Finland",
                    ...
                    print(f"  [FI] Skipping item: {e}")
                    continue

            if page_inserted:
                print(f"  [FI] '{topic[:35]}' s{start} +{page_inserted} "
                      f"(total: {inserted})")
